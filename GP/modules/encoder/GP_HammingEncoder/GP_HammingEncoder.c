...
int parityBitCalc_HammingEncoder(GP_HammingEncoderStruct *GP_HammingEncoder, uint8_t* output, int position)
{
  int count = 0, i, j; // position = 2
    i = position - 1; // i = 2-1 = 1;  
 
    // Traverse to store Hamming Code
    while (i < GP_HammingEncoder->N) {
...
void hammingEncoder(GP_HammingEncoderStruct *GP_HammingEncoder,  uint8_t* input, uint8_t* output){
    // Hamming Encoder Implementation
    int i = 0, j = 0, k = 0;

    // // print the output for debugging
    // printf("For printing out the randomised input bits\n");
    // for (i = 0; i < GP_HammingEncoder->K; i++) {
    // 	printf("input[%d]: %d\n", i, input[i]);
    // }
 
    // Traverse the msgBits
    for (i = 0; i < GP_HammingEncoder->N; i++) {
	  ...
	  //printf("Inside elseeeeeeee\n");
	  output[i] = input[j]; // assign input data
            j++;
        }
    }

    // // print the output for debugging
    // printf("Copy the message bits into the %d-length codeword, with all parity bit positions initialised to 0\n", GP_HammingEncoder->N);
    // for (i = 0; i < GP_HammingEncoder->N; i++) {
    // 	printf("output[%d]: %d\n", i, output[i]);
    // }
    ...
    // hamming code
    for (i = 0; i < GP_HammingEncoder->M; i++) {
 
        // Find current position
        int position = (int)pow(2, i);
 
        // Find value at current position
        int value = parityBitCalc_HammingEncoder(GP_HammingEncoder, output, position);
    ...
    // // print the output for debugging
    // printf("number of codelength, N: %d\n", GP_HammingEncoder->N);
    // for (i = 0; i < GP_HammingEncoder->N; i++) {
    // 	printf("output[%d]: %d\n", i, output[i]);
    // }
}
...
void runGP_HammingEncoder (GP_HammingEncoderStruct *GP_HammingEncoder, signalStruct *signal) {
  ...
  // Computation engine :
  hammingEncoder(GP_HammingEncoder, input, output); // Needa pass in the parameters to the engine
