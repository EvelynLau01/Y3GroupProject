...
// // For calculating the variance of random output generated by SWRC_randn whether is closed to 1
// void  variance_calc(float* arr, int size_arr){
//     int  i, n;
...
//         sum = sum + arr[i];
//     }
//     average = sum / (float)size_arr;
//     /*  Compute  variance  and standard deviation  */
//     for (i = 0; i < size_arr; i++)
//     {
//         sum1 = sum1 + pow((arr[i] - average), 2);
//     }
//     variance = sum1 / (float)size_arr;
...
void awgnChannel(GP_HammingChannelStruct *GP_HammingChannel,  double* input, double* output){
  // SNR input
  double  SNRdB = GP_HammingChannel->SNR;
  double coderate = GP_HammingChannel->R;
  double sigmaNoise, noise;
  int i;
  // int seed = -1;
  // int* seedptr = &seed;
  int idum = 1;
  ...
  printf("sigma noise: %lf\n", sigmaNoise);
  for (i = 0; i < GP_HammingChannel->N; i++) {
  ...
  // for (i = 0; i < GP_HammingChannel->N; i++) {
  //   printf("After Hard-decision (float), output[%d]: %f\n", i, output[i] );
  //   printf("After Hard-decision (uint8_t), output[%d]: %d\n", i, (uint8_t)output[i] );
  // }
}
...
void runGP_HammingChannel (GP_HammingChannelStruct *GP_HammingChannel, signalStruct *signal) {
  ...
  //   // printf(" input[%d]: %d\n", i, input[i]);
  //     random_arr[i] = SWRC_randn(seedptr);
  //     //printf("random_arr[%d], %f\n", i, random_arr[i]);
  // }

  // variance_calc(random_arr, size_arr);

  // +++++++++++++++++++++++++++++++++++++++++++++++++
  // Computation engine :
  double temp_input[N_output];
  ...
  awgnChannel(GP_HammingChannel, temp_input, output); // Should also pass in SNR as input
